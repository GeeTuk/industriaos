...
app.put('/api/pedidos/:id', authMiddleware, (req, res) => {
  ...
  db.prepare(`UPDATE pedidos SET descricao=?, dimensoes=?, material=?, cores=?, prazo=?, valor_orcamento=?, precisa_solvente=?, precisa_uv=?, corte_paralelo=?, status=?, atualizado_em=datetime('now') WHERE id=?`).run(
